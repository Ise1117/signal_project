...
public Alert checkAlert(Patient patient) {
    ...
    double bpm = 60000.0 / averageInterval; 
        ...
        if (bpm > HEARTRATE_TOP) {
            return new Alert(String.valueOf(patient.getId()), "Heart rate too fast, ECG alert", lastRecord.get(lastRecord.size()-1).getTimestamp()) ;
        } else if (bpm < HEARTRATE_BOTTOM) {
            return new Alert(String.valueOf(patient.getId()), "Heart rate too slow, ECG alert", lastRecord.get(lastRecord.size()-1).getTimestamp());
        } 
        ...
        for (long interval : beatIntervals) {
            if (interval < lowerThreshold || interval > upperThreshold) {
                return new Alert(String.valueOf(patient.getId()), "ECG strange pattern Alert", lastRecord.get(lastRecord.size()-1).getTimestamp());
